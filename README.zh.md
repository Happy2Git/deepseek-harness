...
面板的宿主路由仅限回环、请求体有上限、只接受完全限定路径，超限或格式错误的读取失败即关闭；git 只读，图片拖入从不复制进工作区。上下文标签的文档流做了签名门控，只在文档真正变化时重渲染；完整历史经 `/dir/injected-docs` 在服务端过滤持久化日志获取，在 18 万事件的会话上把每次激活约 120 MB 的历史页 JSON 换成单次 KB 级响应。所有读取与列举都有界并随调用方取消。
