...
- **带外完整历史** — 上下文标签页实时重投影(会话切换、事件流前进或显式点击刷新都会触发),并在会话激活时经插件自有的 `/dir/injected-docs` 路由一次性取回该会话的注入文档源事件(宿主在服务端筛持久化日志,工具载荷绝不跨线),用同一套出处投影折叠;两个区块因此持有完整日志,同时不动共享的对话窗口(聊天视图保留自己的「加载更早」与滚动锚点)。
