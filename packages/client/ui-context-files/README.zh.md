...
- **有界带外历史拉取** — 上下文标签页实时重投影(会话切换、事件流前进或显式点击刷新都会触发),并在会话激活时经 connection 的历史 RPC 拉取更早的历史页——最多 20 页(1,000 条消息),在面板内私有折叠——两个区块因此持有完整日志,同时不动共享的对话窗口(聊天视图保留自己的「加载更早」);更深的历史仍留给手动翻页控件。
