...
export const zh = {
  ...
  'panelImage.attached': '已拖入图片 {path}，图片内容已附在本条消息中。',
  'panelImage.pathOnly': '已拖入图片 {path}。',
  'panelImage.unreadable': '图片读取失败，文件可能已移动或格式不支持',
  'file.tooLarge': '文件 "{name}" 过大，未插入',
  'file.tooMany': '一次最多添加 20 个文件，未插入',
  'file.totalTooLarge': '文件总大小超过 1MB，未插入',
  'file.unreadable': '无法读取文件内容，未插入',
...
export const en = {
  ...
  'panelImage.attached': 'Dropped image {path}; its content is attached to this message.',
  'panelImage.pathOnly': 'Dropped image {path}.',
  'panelImage.unreadable': 'Could not read the image; it may have moved or use an unsupported format',
  'file.tooLarge': 'File "{name}" is too large to insert',
  'file.tooMany': 'You can add at most 20 files at a time, so none were inserted',
  'file.totalTooLarge': 'The total file size exceeds 1MB, so the files were not inserted',
  'file.unreadable': 'Could not read the file content, so it was not inserted',
