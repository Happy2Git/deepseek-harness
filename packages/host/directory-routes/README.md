...
Plugin-owned HTTP routes for the context-files panel's directory browsing. The browser panel cannot reach `ctx.directoryPicker` directly, and the core API gateway is a closed contract, so this plugin carries the panel's transport with it: it registers four exact routes on `ctx.webServer` and answers each from the composed `ctx.directoryPicker` browse capability.
